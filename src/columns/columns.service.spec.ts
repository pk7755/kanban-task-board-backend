...
const mockColumnA = { id: COL_A, name: 'To Do', boardId: BOARD_ID, position: 1, color: null, createdAt: new Date(), updatedAt: new Date() };
const mockColumnB = { id: COL_B, name: 'Done', boardId: BOARD_ID, position: 2, color: null, createdAt: new Date(), updatedAt: new Date() };
...
describe('ColumnsService', () => {
  ...
  describe('create', () => {
    it('✅ creates column at position maxPos + 1', async () => {
      ...
      mockPrisma.column.create.mockResolvedValue({ ...mockColumnA, position: 3 });

      const result = await service.create({ name: 'In Progress', boardId: BOARD_ID }, OWNER_ID);
    ...
    it('✅ creates first column at position 1 when board is empty', async () => {
      ...
      mockPrisma.column.create.mockResolvedValue({ ...mockColumnA, position: 1 });
    ...
    it('✅ stores optional color when provided', async () => {
      ...
      mockPrisma.column.create.mockResolvedValue({ ...mockColumnA, color: '#FF5733' });

      await service.create({ name: 'To Do', boardId: BOARD_ID, color: '#FF5733' }, OWNER_ID);
  ...
  describe('update', () => {
    it('✅ updates column name', async () => {
      ...
      mockPrisma.board.findUnique.mockResolvedValue(mockBoard);
      mockPrisma.column.update.mockResolvedValue({ ...mockColumnA, name: 'Renamed' });
    ...
    it('✅ updates color only when provided', async () => {
      mockPrisma.column.findUnique.mockResolvedValue(mockColumnA);
      mockPrisma.board.findUnique.mockResolvedValue(mockBoard);
      mockPrisma.column.update.mockResolvedValue({ ...mockColumnA, color: '#00FF00' });
    ...
    it('✅ updates position when provided', async () => {
      mockPrisma.column.findUnique.mockResolvedValue(mockColumnA);
      mockPrisma.board.findUnique.mockResolvedValue(mockBoard);
      mockPrisma.column.update.mockResolvedValue({ ...mockColumnA, position: 5 });
    ...
    it('❌ throws NotFoundException when column does not exist', async () => {
      ...
      await expect(service.update('bad-col', { name: 'X' }, OWNER_ID)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('❌ throws ForbiddenException when requester is not the board owner', async () => {
      ...
      await expect(service.update(COL_A, { name: 'Hack' }, NON_OWNER_ID)).rejects.toThrow(
        ForbiddenException,
      );
    });
  });
  ...
  describe('remove', () => {
    it('✅ deletes the column and returns success message', async () => {
      ...
      expect(mockPrisma.column.delete).toHaveBeenCalledWith({ where: { id: COL_A } });
    ...
    it('❌ throws ForbiddenException when non-owner tries to delete', async () => {
      ...
      await expect(service.remove(COL_A, NON_OWNER_ID)).rejects.toThrow(ForbiddenException);
    });

    it('❌ throws NotFoundException when column does not exist', async () => {
      mockPrisma.column.findUnique.mockResolvedValue(null);

      await expect(service.remove('bad-col', OWNER_ID)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('reorder', () => {
    ...
    it('❌ throws BadRequestException for an empty items array', async () => {
      await expect(service.reorder([], OWNER_ID)).rejects.toThrow(BadRequestException);
    });

    it('❌ throws BadRequestException when there are duplicate column IDs', async () => {
      await expect(
        service.reorder([{ id: COL_A, position: 1 }, { id: COL_A, position: 2 }], OWNER_ID),
      ).rejects.toThrow(BadRequestException);
    });

    it('❌ throws BadRequestException when there are duplicate positions', async () => {
      await expect(
        service.reorder([{ id: COL_A, position: 1 }, { id: COL_B, position: 1 }], OWNER_ID),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('❌ throws NotFoundException when one or more columns are not found', async () => {
      ...
      await expect(service.reorder(validItems, OWNER_ID)).rejects.toThrow(NotFoundException);
    });
    ...
    it('❌ throws BadRequestException when columns belong to different boards', async () => {
      ...
      await expect(service.reorder(validItems, OWNER_ID)).rejects.toThrow(BadRequestException);
    });
    ...
    it('❌ throws ForbiddenException when requester is not the board owner', async () => {
      ...
      await expect(service.reorder(validItems, NON_OWNER_ID)).rejects.toThrow(ForbiddenException);
    });
  });
});
